boxplotfeats = function(data, feature, cluster_labels){
  ...
  plot = ggplot(as.data.frame(as.numeric(y)), aes_string(x = factor(cluster_labels), y = as.numeric(y), fill=factor(cluster_labels))) + geom_boxplot()  + stat_compare_means(method = "kruskal") + 
